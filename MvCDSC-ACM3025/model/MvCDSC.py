...
class MvCDSC():
    ...
    def __call__(self, A, X, R, S, p, y_pred, A2, X2, R2, S2, Theta):
        ...
        Z2 = self.Z2
        for layer in range(self.n_layers3):
            H32 = self.__encoder32(A2, H32, layer)
        self.H32 = H32
        ...
        self.S_emb32 = tf.nn.embedding_lookup(self.H32, S2)
        self.R_emb32 = tf.nn.embedding_lookup(self.H32, R2)
    ...
    def __encoder31(self, A, H, layer):
        ...
        return tf.matmul(self.C31[layer], H)
    ...
    def __encoder32(self, A, H, layer):
        ...
        return tf.matmul(self.C32[layer], H)
