...
from utils.visualize import visualize_data_tsne
...
class Trainer():
    ...
    def optimize(self, loss, vars_7, vars_6):
        ...
        optimizer2 = tf.train.AdamOptimizer(learning_rate=2e-4)
    ...
    def __call__(self, A, X, S, R, A2, X2, S2, R2, y_true, num_clusters):
        ...
        y_x, _ = post_proC(commonZ, self.args.cluster, 10, 3.5)
        ...
        print("acc: {:.8f}\t\tnmi: {:.8f}\t\tf_score: {:.8f}\t\tari: {:.8f}".
              format(cluster_acc(y_true, y_x - 1), nmi(y_true, y_x - 1), f_score(y_true, y_x - 1),
                     ari(y_true, y_x - 1)))
        print("-------------------------------------------------------------")
        plot_path = "/home/aiuser1/code/visualization/"        # 可视化的路径
        ...
        for epoch in range(self.args.n_epochs):
            ...
            # visualization
            # if (epoch == 0) or (epoch == 13) or (epoch == 35):
            #     visualize_data_tsne(coef3, y_true, self.args.cluster, plot_path + "acm{}.png".format(epoch))
